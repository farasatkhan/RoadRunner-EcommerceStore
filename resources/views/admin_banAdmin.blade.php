...
<nav class="navbar navbar-light navbar-expand-md navbar-properties-custom">
                        ...
                        <div class="container">
                            <div class="row">
                                <div class="subLogin offset-lg-2 col-lg-8 offset-mg-4 col-mg-8 col-10 offset-1">
                                    <form id="ban-admin-account-form" action="/admin_banAdmin" method="POST">
                                        @csrf
                                        <p class="admin-newadmin-form-title">Ban Existing Admin</p>
                                        <input  id=email name=email type="text" class="formTextFeild" placeholder="Email" required>
                                        <p id="email-validation-error"></p>
                                        <input type="submit" name="Login" id="Login" value="Ban Existing Admin" class="btn1">
                                    </form>
                                    <?php
                                        if (!empty($status)){
                                            if(str_contains($status, "ERROR")){
                                                print '<p style="color:red; text-align:center;">'. $status .'</p>';
                                            }else{
                                                print '<p style="color:green; text-align:center;">'. $status .'</p>';
                                            }
        ...
        <script>
            ...
            var form = document.getElementById("ban-admin-account-form");
